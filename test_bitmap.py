...
def image_to_binary_bitmap(image):
    ...
    transposed_binary = [binary_string[i:i+height] for i in range(0, len(binary_string), height)]
    hex_values = [','.join([f'0X{int(pixel, 2):02X}' for pixel in row]) for row in zip(*transposed_binary)]
